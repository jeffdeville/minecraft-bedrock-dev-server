...
One installable package for a fresh Ubuntu VPS. After `bootstrap-course.sh`
...
**Deploy-on-save: the existing deploy path, per learner, with no rsync.** An
`inotifywait` sidecar per learner watches `workspace/packs/`, waits two
seconds of quiet, and runs the same `install-packs.py` and restart logic that
`bin/deploy.sh` runs today, against that learner's data directory and
container. Half-saved JSON fails loudly and the next save retries, exactly as
now. The sidecar needs no Docker socket, only the bind-mounted data directory;
the control plane is the only process that talks to Docker.
...
platform/                    Thread B. Everything that runs on the box.
  bootstrap-course.sh        fresh Ubuntu -> running stack (one command, idempotent)
  docker-compose.yml         caddy + control plane
  learner.compose.yml        template: bds + code-server + deploy sidecar
  Caddyfile.tmpl             routes; per-learner block generated by the control plane
  control/                   Python stdlib service: auth, provision, dashboard, SQLite
...
bin/deploy.sh, bin/install-packs.py   parameterised so the sidecar reuses them
```

The existing `bin/mc` loop keeps working unchanged: `bin/deploy.sh` grows
optional environment variables for container name, data and state directories,
with defaults equal to today's values.
...
- **B1. Parameterise the deploy path.** `bin/deploy.sh` and the compose
  invocation take `MC_CONTAINER`, `DATA`, `STATE`, `ADDONS`, `LEVEL_NAME` from
  the environment with today's values as defaults. `install-packs.py` already
  takes them positionally. Verify with `bash -n`, `py_compile`, and a normal
  `mc sync` to the live server still working. Done when the diff to the
  existing path is defaults only.
...
- **B6. Caddy.** `Caddyfile.tmpl` with `forward_auth` on `/u/*`, a generated
  ...
  on a real hostname.
- **B7. Packaging.** `platform/bootstrap-course.sh`: install Docker from
  Docker's apt repo, unpack to `/opt/mc-course`, write the systemd unit,
  `compose up`, print the admin URL and a generated admin password. A `make
  package` target produces the tarball from the branch. Debian packaging and
...
- **The grader pack.** Content owns what `PASS <lesson>` means; the platform
  owns sending `scriptevent course:check <lesson>` and grepping the log.
